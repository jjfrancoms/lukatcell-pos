...
export default function Layout() {
  ...
  return (
    <div className={`flex h-screen bg-[#0d1117] ${roleClass}`}>
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[#010409] border-b border-[#30363d] flex items-center justify-between px-4 pb-3" style={{ paddingTop: 'calc(0.75rem + env(safe-area-inset-top))' }}>
        <div className="flex items-center gap-2"><div className="w-7 h-7 rounded-lg bg-gradient-to-br from-cyan-400 to-cyan-600 flex items-center justify-center"><Smartphone size={14} className="text-black" /></div><div><span className="font-display font-bold text-sm text-white block leading-none">LUKATCELL</span><EstadoBadge compact /></div></div>
        <div className="flex items-center gap-3">{!online && <WifiOff size={16} className="text-orange-400" aria-label="Sin conexión" />}<button onClick={() => setMenuOpen(!menuOpen)} className="text-gray-400" aria-label={menuOpen ? 'Cerrar menú' : 'Abrir menú'}>{menuOpen ? <X size={22} /> : <Menu size={22} />}</button></div>
      </div>
