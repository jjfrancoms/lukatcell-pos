...
assert(app.includes('path="permisos" element={<AdminRoute><PermisosPersonal /></AdminRoute>}'), 'Permisos esta protegido por AdminRoute')
...
assert(layout.includes("{ to: '/permisos', label: 'Permisos'"), 'Permisos solo se agrega en navegacion administrativa')
assert(layout.includes("{ to: '/auditoria', label: 'Auditoría'"), 'Auditoria solo se agrega en navegacion administrativa')
assert(permisos.includes("rpc('registrar_permiso_personal'"), 'Permisos se registran mediante RPC validada')
assert(permisos.includes("rpc('cancelar_permiso_personal'"), 'Permisos se cancelan mediante RPC validada')
assert(permisos.includes("rpc('permisos_personal_admin'"), 'Listado de permisos usa RPC administrativa')
assert(!permisos.includes("from('personal_permisos').insert("), 'UI no inserta permisos directamente')
assert(!permisos.includes("from('personal_permisos').update("), 'UI no actualiza permisos directamente')
assert(jornada.includes("['permiso', 'vacaciones', 'licencia']"), 'Mi Jornada reconoce permisos, vacaciones y licencias')
assert(jornada.includes('!esPermiso'), 'Mi Jornada oculta registro de entrada durante permisos')
