...
assert(app.includes('path="cambios-turno" element={<AdminRoute><CambiosTurno /></AdminRoute>}'), 'Cambios de turno esta protegido por AdminRoute')
assert(app.includes('path="anulaciones" element={<AdminRoute><Anulaciones /></AdminRoute>}'), 'Anulaciones esta protegido por AdminRoute')
...
assert(layout.includes("{ to: '/cambios-turno', label: 'Cambios de turno'"), 'Cambios de turno solo se agrega en navegacion administrativa')
assert(layout.includes("{ to: '/anulaciones', label: 'Anulaciones'"), 'Anulaciones solo se agrega en navegacion administrativa')
...
assert(cambiosTurno.includes("rpc('registrar_excepcion_turno'"), 'Cambios temporales se registran mediante RPC validada')
assert(cambiosTurno.includes("rpc('cancelar_excepcion_turno'"), 'Cambios temporales se cancelan mediante RPC validada')
assert(cambiosTurno.includes("rpc('excepciones_turno_admin'"), 'Listado de cambios temporales usa RPC administrativa')
assert(!cambiosTurno.includes("from('staff_turno_excepciones').insert("), 'UI no inserta excepciones de turno directamente')
assert(!cambiosTurno.includes("from('staff_turno_excepciones').update("), 'UI no actualiza excepciones de turno directamente')
assert(anulaciones.includes("rpc('anular_venta'"), 'Anulaciones usa RPC transaccional del servidor')
assert(!anulaciones.includes("from('sales').update("), 'UI de anulaciones no modifica ventas directamente')
assert(!anulaciones.includes("from('inventory').update("), 'UI de anulaciones no repone stock directamente')
...
assert(dashboard.includes('personal_permisos'), 'Dashboard muestra conteo de personal con permiso')
assert(dashboard.includes("p.estado === 'permiso'"), 'Dashboard representa permiso como estado propio')
assert(dashboard.includes("p.estado === 'vacaciones'"), 'Dashboard representa vacaciones como estado propio')
assert(dashboard.includes("p.estado === 'licencia'"), 'Dashboard representa licencia como estado propio')
