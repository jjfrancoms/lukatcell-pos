...
export default function Caja() {
  ...
  const cerrarTurno = async () => {
    if (!sesionActiva) return
    setCargando(true)
    const { error } = await supabase.from('cash_sessions').update({ cierre: new Date().toISOString(), monto_final_esperado: montoEsperado, monto_final_contado: Number(montoContado) || 0 }).eq('id', sesionActiva.id)
    setCargando(false)
