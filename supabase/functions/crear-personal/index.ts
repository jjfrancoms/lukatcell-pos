...
Deno.serve(async (req: Request) => {
  ...
  const { data: callerStaff, error: callerStaffErr } = await admin
    .from('staff').select('rol, activo, location_id').eq('user_id', userData.user.id).maybeSingle()
  if (callerStaffErr || !callerStaff || callerStaff.rol !== 'administrador' || !callerStaff.activo) {
    return json({ error: 'Solo un administrador puede crear personal' }, 403)
  }
  ...
  if (!nombre || !username || !correo || password.length < 6) {
    return json({ error: 'Nombre, usuario, correo y una contraseña de al menos 6 caracteres son obligatorios' }, 400)
  }
  if (!EMAIL_RE.test(correo)) return json({ error: 'El correo no es válido (ej: nombre@correo.com)' }, 400)
  if (!/^[a-z0-9._-]{3,}$/.test(username)) {
    return json({ error: 'El usuario debe tener al menos 3 caracteres (letras, números, puntos o guiones)' }, 400)
  }
  ...
  const { data: staffRow, error: staffErr } = await admin.from('staff').insert({
    ...
    rol,
    puesto,
    location_id: callerStaff.location_id,
    activo: true,
  }).select().single()
